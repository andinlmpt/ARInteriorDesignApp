/**
 * AR View furniture placement — add, remove, restore, collision checks.
 */
...
import { MAX_CACHE_SIZE, MAX_POOL_SIZE } from '@/constants/ar-config';
...
import { AR_VIEW_ERROR_MESSAGES } from '@/config/arView.messages';

export interface UseARViewFurniturePlacementParams {
  ...
  geometryCacheRef: React.MutableRefObject<Map<string, THREE.BufferGeometry>>;
  materialCacheRef: React.MutableRefObject<Map<string, THREE.Material>>;
  geometryPoolRef: React.MutableRefObject<Map<string, THREE.BufferGeometry[]>>;
  materialPoolRef: React.MutableRefObject<Map<string, THREE.Material[]>>;
  ...
  furnitureLibraryById: Record<string, FurnitureLibraryItem>;
  selectedLibraryItem: string | null;
  placedFurniture: PlacedFurnitureMeta[];
  roomData: RoomData | null;
  placementSafety: { safetyScore: number };
  ...
  validateHistoryState: (state: PlacedFurnitureMeta[], checkScenePresence?: boolean) => PlacedFurnitureMeta[];
  clearHistory: () => void;
}

export function useARViewFurniturePlacement({
  furnitureMapRef,
  obstacleMapRef,
  rootGroupRef,
  sceneRef,
  ...
  clearHistory,
}: UseARViewFurniturePlacementParams) {
  ...
  const addFurnitureToScene = useCallback(
    async (worldPosition: THREE.Vector3) => {
      ...
      try {
        ...
        const furnitureGroup = await furnitureModelLoader.createDetailedFurnitureModel(libraryItem);
        ...
        setComponentError(AR_VIEW_ERROR_MESSAGES.FURNITURE_PLACEMENT_ERROR);
        Alert.alert('Placement Error', AR_VIEW_ERROR_MESSAGES.FURNITURE_PLACEMENT_ERROR, [
